 ...
 *  MaxIter [int 1 x 1] max number of iterations.
...
libqp_state_T libqp_splx_solver(const double *(*get_col)(int), double *diag_H,
  ...
  for (i = 0, m = 0; i < n; i++) m = LIBQP_MAX(m, I[i]);
  ...
  inx = (int *)LIBQP_CALLOC(m * n, sizeof(int));
  if (inx == NULL) {
    state.exitflag = -1;
    goto cleanup;
  }
  ...
  nk = (int *)LIBQP_CALLOC(m, sizeof(int));
  if (nk == NULL) {
    state.exitflag = -1;
    goto cleanup;
  }
  ...
  /* print initial state */
  if (print_state != NULL) print_state(state);
  ...
  while (state.exitflag == 100) {
      ...
      if (delta > TolAbs / m && delta > TolRel * LIBQP_ABS(state.QP) / m) {
        ...
        if (u != -1) {
          ...
          if (v != -1) {
            ...
            for (i = 0; i < n; i++) d[i] += tmp * (col_u[i] - col_v[i]);
          } else {
            tmp = x_neq[k] * tau;
            x[u] += tmp;
            x_neq[k] -= tmp;
    ...
    if (state.QP - state.QD <= LIBQP_ABS(state.QP) * TolRel)
      state.exitflag = 1;
    else if (state.QP - state.QD <= TolAbs)
      state.exitflag = 2;
    else if (state.QP <= QP_TH)
      state.exitflag = 3;
    else if (state.nIter >= MaxIter)
      state.exitflag = 0;
  }
