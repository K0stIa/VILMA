...
template <class Loss>
VilmaOracle::PwMOrd<Loss>::PwMOrd(Data *data,
                                  const std::vector<int> &cut_labels)
    : PwMOrdRegularized<Loss>(data, cut_labels),
...
template <class Loss>
double VilmaOracle::PwMOrd<Loss>::risk(const double *weights, double *subgrad) {
...
template <class Loss>
std::vector<double> VilmaOracle::PwMOrd<Loss>::Train() {
